<template>
  <div id="minwidth">
    <el-card class="box-card">
      ...
      <!-- 查看新闻详情的对话框 -->
      <!-- 查看新闻详情的对话框 -->
      <el-dialog title="新闻预览" :visible.sync="detailVisible" width="80%">
...
export default {
  ...
  data () {
    return {
      ...
      pubDialogVisible: false, // 控制发表新闻对话框的显示与隐藏
      pubForm: {
        // 表单的数据对象
        title: '',
        cate_id: '',
      ...
      pubFormRules: {
        ...
        title: [
          { required: true, message: '请输入新闻标题', trigger: 'blur' },
          {
            min: 1,
            max: 30,
            message: '新闻标题的长度为1-30个字符',
            trigger: 'blur'
          }
        ],
        cate_id: [
          { required: true, message: '请选择新闻标题', trigger: 'blur' }
        ],
        content: [
      ...
      artList: [], // 新闻的列表数据
      total: 0, // 总数据条数
      detailVisible: false, // 控制新闻详情对话框的显示与隐藏
      artDetail: {}, // 新闻的详情信息对象
      baseURL // 基地址
    }
  },
  ...
  methods: {
    ...
    pubArticleFn (state) {
      ...
      this.$refs.pubFormRef.validate(async (valid) => {
        if (!valid) return this.$message.error('请完善新闻信息！')
        // 3. 判断是否提供了新闻封面
        if (!this.pubForm.cover_img) {
          return this.$message.error('请选择新闻封面！')
        }
        // 4. TODO：发布新闻
        // 创建 FormData 对象
        ...
        if (res.code !== 0) return this.$message.error('发布新闻失败！')
    ...
    async initArtListFn () {
      ...
      if (res.code !== 0) return this.$message.error('获取新闻列表失败!')
    ...
    async showDetailFn (id) {
      ...
      if (res.code !== 0) return this.$message.error('获取新闻详情失败!')
